...
export class EventEditComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const eventData: Event = {
      ...
      ville: this.eventForm.value.ville || undefined,
      pays: this.eventForm.value.pays || undefined,
      meetingLink: this.eventForm.value.meetingLink || undefined,
  ...
  addParticipant(): void {
    ...
    this.eventService.addParticipant(this.eventId, this.selectedParticipantToAdd).subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: 'Succès',
  ...
  confirmRemoveParticipant(participant: Participant): void {
    this.confirmationService.confirm({
      message: `Êtes-vous sûr de vouloir retirer ${participant.firstName} ${participant.lastName} de cet événement ?`,
      header: 'Confirmation',
      ...
      this.fileService.uploadFile(formData).subscribe({
        ...
        error: () => {
          this.messageService.add({
            severity: 'error',
            summary: 'Erreur',
            detail: `Impossible d'ajouter le fichier "${file.name}"`
          });
        }
      });
    }
  ...
  confirmDeleteFile(file: FileUpload): void {
    this.confirmationService.confirm({
      message: `Êtes-vous sûr de vouloir supprimer le fichier "${file.fileName}" ?`,
      header: 'Confirmation',
      icon: 'pi pi-exclamation-triangle',
