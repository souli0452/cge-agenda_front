...
<input 
pInputText 
[(ngModel)]="newParticipant.phoneNumber" 
class="w-full" 
placeholder="+226..." />
</div>
</div>

<!-- Ligne 3: Structure | Fonction -->
<div class="flex gap-3 mb-3">
<div class="flex-1">
<label class="block mb-2 font-semibold text-sm">Structure</label>
<input 
pInputText 
[(ngModel)]="newParticipant.structure" 
class="w-full" 
...
<input 
pInputText 
[(ngModel)]="newParticipant.jobTitle" 
class="w-full" 
placeholder="Fonction" />
</div>
</div>

<!-- Ligne 4: Type (pleine largeur) -->
<div class="mb-3">
...
if (currentStatus === 'ANNULE' || currentStatus === 'TERMINE') return;
...
const updatedEvent: Event = { ...event, status: newStatus as EventStatus };
hasChanges = true;
this.eventService.updateEvent(event.id!, updatedEvent).subscribe({
next: () => event.status = newStatus as EventStatus
});
}
});
if (hasChanges) this.applyFilters();
}
...
severity: 'warn',
summary: 'Action non autorisée',
detail: 'Impossible de gérer les fichiers pour un événement terminé ou annulé'
});
return;
}
...
exportEventsToPdf(): void {
  ...
  this.eventService.exportMonthlyPdf(
    ...
    next: (blob: Blob) => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `evenements_${today.getFullYear()}_${today.getMonth() + 1}.pdf`;
      a.click();
      window.URL.revokeObjectURL(url);
      ...
      this.messageService.add({
        severity: 'success',
        summary: 'Succès',
        detail: 'Export PDF généré avec succès'
      });
    },
    error: (err: any) => {
      ...
      this.messageService.add({
        severity: 'error',
        summary: 'Erreur',
        detail: 'Impossible d\'exporter la liste des événements'
      });
    }
  });
}
